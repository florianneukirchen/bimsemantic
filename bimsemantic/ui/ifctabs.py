...
class IfcTabs(QWidget):
    ...
    def __init__(self, parent):
        ...
        self.mainwindow.column_treemodel.hideInfoColumn.connect(
    ...
    def update_columns(self):
        ...
        self.remaining_models = [
            self.tabs.widget(i).treemodel for i in range(self.tabs.count())
        ]
    ...
    def update_next_model(self):
        ...
        if self.remaining_models:
            ...
            self.mainwindow.progressbar.setValue(
                self.mainwindow.progressbar.value() + 1
            )
        else:
            self.timer.stop()
    ...
    def make_custom_tab(self, name, custom_fields):
        ...
        custom_tab.close_button.clicked.connect(
            lambda: self.remove_custom_tab(custom_tab)
        )

        tab_index = self.tabs.indexOf(custom_tab)
        self.tabs.tabBar().setTabButton(
            tab_index, QTabBar.RightSide, custom_tab.close_button
        )
...
class IfcTreeTab(QWidget):
    ...
    def on_selection_changed(
        ...
        if len(items) > 1:
            self.mainwindow.statusbar.showMessage(
                self.tr("%i items selected") % len(items)
            )
        else:
            self.mainwindow.statusbar.clearMessage()
    ...
    def copy_selection_to_clipboard(self):
        ...
        txt = "".join(
            self.rows_to_csv(sep=";", add_header=add_header, add_level=add_level)
        )
        ...
        if index.isValid() and index.column() > 0:
            context_menu.addAction(
                QAction(
                    ...
                    self,
                    triggered=lambda: self.treemodel.columntree.remove_column(
                        index.column()
                    ),
                )
            )
