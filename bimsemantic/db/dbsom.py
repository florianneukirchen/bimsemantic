...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p",
        "--path",
        help="Pfad zum Ordner mit CSV-Dateien (default: aktuelles Arbeitsverzeichnis)",
        default=None,
    )
    parser.add_argument(
        "-o",
        "--output",
        help="Dateiname für die JSON-Datei (default: som.json)",
        default="db_som.json",
    )
    parser.add_argument(
        "-e",
        "--encoding",
        help="Encoding der CSV-Dateien (default: iso-8859-1)",
        default="iso-8859-1",
    )
    parser.add_argument(
        "-d",
        "--delimiter",
        help="Trennzeichen in der CSV-Datei (default: ;)",
        default=";",
    )

    args = parser.parse_args()

    som = folder_csv_files_to_som_dict(
        path=args.path, encoding=args.encoding, delimiter=args.delimiter
    )
